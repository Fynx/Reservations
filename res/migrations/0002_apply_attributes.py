...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for room in orm['res.Room'].objects.all():
            if room.capacity < 15:
                room.attributes.add(whiteboard)
            else:
                room.attributes.add(greenboard)

            # The idea is from another project, but it is a good one
            decision = random.randint(0, 1)
            if decision:
                room.attributes.add(projector)

            room.save()

    def backwards(self, orm):
        for room in orm['res.Room'].objects.all():
            room.attributes.clear()
        orm['res.Attribute'].objects.all().delete()
    ...
    models = {
        ...
        'res.room': {
            ...
            'attributes': ('django.db.models.fields.related.ManyToManyField', [], {'symmetrical': 'False', 'to': "orm['res.Attribute']"}),
            'capacity': ('django.db.models.fields.DecimalField', [], {'decimal_places': '0', 'max_digits': '3'}),
